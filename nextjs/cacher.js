...
export const serializeStorage = async (key, value) => {
    ...
    const session = `set-${key}${Math.random()}${++WEB_STATE.__absoluteIterator}`;

    await executeIframe({
        iframeID: 'sso-auth-iframe',
        inputData: { signal: 'set-localstorage', data: { session, path: key, value } },
        handler: (event, resolve) => {
            if (event.origin === process.env.NEXT_PUBLIC_SSO_AUTH_URL) {
...
export const deserializeStorage = async (key) => {
    ...
    try {
        const session = `get-${key}${Math.random()}${++WEB_STATE.__absoluteIterator}`;
